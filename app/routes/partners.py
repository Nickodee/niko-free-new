...
from app.models.partner import Partner, PartnerSupportRequest, PartnerTeamMember, PartnerStaff
...
def create_event(current_partner):
    ...
    if poster_file:
        try:
            print(f"[EVENT CREATE] Uploading poster image: {poster_file.filename}")
            poster_path = upload_file(poster_file, folder='events')
            if poster_path:
                print(f"[EVENT CREATE] ✅ Poster uploaded successfully: {poster_path}")
            else:
                print(f"[EVENT CREATE] ⚠️ Poster upload returned None")
        except ValueError as e:
            print(f"[EVENT CREATE] ❌ Error uploading poster: {e}")
            return jsonify({'error': str(e)}), 400
        except Exception as e:
            print(f"[EVENT CREATE] ❌ Unexpected error uploading poster: {e}")
            import traceback
            traceback.print_exc()
            # Don't fail event creation if poster upload fails, but log it
    ...
    # Log poster image path for debugging
    if poster_path:
        print(f"[EVENT CREATE] Event created with poster_image: {poster_path}")
    else:
        print(f"[EVENT CREATE] Event created without poster_image")
...
def get_team_members(current_partner):
    """Get active team members (staff) for this partner"""
    members = PartnerStaff.query.filter_by(
        partner_id=current_partner.id,
        is_active=True
...
def remove_team_member(current_partner, member_id):
    """Remove a team member (staff)"""
    member = PartnerStaff.query.filter_by(
        id=member_id,
        partner_id=current_partner.id,
        is_active=True
    ).first()
