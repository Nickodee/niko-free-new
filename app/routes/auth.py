...
def partner_login():
    ...
    # Import PartnerStaff here to avoid import errors if model doesn't exist
    try:
        from app.models.partner import PartnerStaff
        PartnerStaff_available = True
    except ImportError as e:
        # If PartnerStaff doesn't exist, staff login is not supported
        current_app.logger.warning(f'PartnerStaff model not available: {e}')
        PartnerStaff_available = False
        PartnerStaff = None
    
    if PartnerStaff_available:
        user = User.query.filter_by(email=email).first()
