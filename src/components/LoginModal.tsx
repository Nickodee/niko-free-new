...
export default function LoginModal({ isOpen, onClose, onNavigate }: LoginModalProps) {
  ...
  // Initialize Google Sign-In when modal opens (but don't render the default button)
  useEffect(() => {
    if (!isOpen) return;

    // We always use our custom styled button instead of Google's default button
  }, [isOpen]);
