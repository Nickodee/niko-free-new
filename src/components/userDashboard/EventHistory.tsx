...
export default function EventHistory({ onEventClick, onBack, onNavigate }: EventHistoryProps) {
    ...
    try {
      ...
      const formattedEvents: Event[] = (response.bookings || []).map((booking: any) => {
        ...
        return {
          ...
          partnerId: event.partner_id || event.partner?.id
        };
      });
                ...
                {event.partnerId && (
                  <button 
                    onClick={(e) => {
                      e.stopPropagation();
                      if (onNavigate) {
                        onNavigate('partner-profile', { partnerId: event.partnerId });
                      }
                    }}
                    className="w-full py-2 bg-[#27aae2] text-white rounded-lg text-sm font-semibold hover:bg-[#1e8bb8] transition-colors flex items-center justify-center space-x-2"
                  >
